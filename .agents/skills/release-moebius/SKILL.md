...
- Rebuild the final ZIP and DMG from this stapled App using electron-builder `--prepackaged` **with the `.app` bundle path itself** (e.g. `--prepackaged release/mac-arm64/Moebius.app`). Passing a directory that merely contains the `.app` (e.g. `release/mac-arm64`) makes electron-builder nest the bundle (`Moebius.app/Moebius.app/...` in the DMG and a `mac-arm64/` top-level in the ZIP), which macOS rejects as an invalid bundle (Finder shows a stop icon) and breaks Squirrel updates. After the rebuild, verify the DMG mounts with a single-level `Moebius.app/Contents` and the ZIP's top-level entry is `Moebius.app/` before proceeding. Do not upload artifacts produced before the App was stapled.
