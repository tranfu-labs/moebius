...
- **两者职责不混**：工具条属于**消息**，管「查看这条消息」（完整输出、更多操作），每条 Agent 消息都有、位置一致，成功失败都一样；报错卡片属于**这次运行**，管「它挂了，怎么救」（重试、换执行配置、迁移会话、前往设置）。查看类动作不进卡片，恢复类动作不进工具条。
- **报错卡片**：`incident-card.tsx`。运行出了事，就在那条消息**下面紧跟一张独立卡片**（描边 + `bg-sunken`，严重态换 `border-danger`），当场写清状态名、耗时、引擎诊断与「内容不完整」提示，并把**带文字的「重试」**放在卡片里，次级恢复动作退为图标。不要把事故折叠成工具条上的一个孤立标记——用户看不懂一个没有上下文的感叹号。卡片只跟在消息后面，永远不接管消息正文。用户自己发起的停止不算事故：不出卡片，恢复动作留在工具条里。
