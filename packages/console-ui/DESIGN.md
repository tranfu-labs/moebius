...
- **画像选择器**：`src/console/agent-portrait-picker.tsx`——触发器就是当前成员的画像本身，不加独立按钮；popover 内 6×6 平铺整池，**每个候选都用该成员现有的身份色渲染**（底色不可选，中性底色挑完再上色会让一半候选是假预览）。选中项用 `border-accent` 环，roving tabindex + 方向键把 36 个候选收成一个 tab stop（`role="radiogroup"` / `role="radio"`）。候选恒定 40px（六列 328px 面），不靠 hover 放大来补可读性——28px 下画像风格本身就会失效（见 `.claude/skills/generate-avatar-set/`），而 hover 对键盘与触摸不存在，最需要看清的人恰好拿不到；邻项缩放也已在目录轨与侧栏信息面两处禁止。格子用 `aspect-square w-full` 跟随栅格列宽，不写死宽度，否则列宽一变就溢出。选中默认那张时回调 `null` 而不是它的 id，否则「跟随默认」会被冻成一次显式选择；「恢复默认」在没有存过选择时 disabled。存的是稳定 id 不是下标——扩池会让下标整体错位；读到池里不存在的 id 时回落默认而不是渲染空框。
