...
- **Claude 原生信任提示**：Claude Code 的原生信任、认证或权限失败由 headless runtime 直接归类为结构化错误；组件库只显示结构化活动、错误与原生历史结果，不渲染信任弹层、信任／拒绝按钮或可编辑的原生终端输入。原生过程输出不能成为组件侧的决策依据，正常 Agent Markdown 仍由最终结果流程产生。
