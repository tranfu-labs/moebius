...
- **团队版本追溯与应用**：`src/console/agent-team-option.tsx`、`session-team-update-notice.tsx`、`agent-run-info-popover.tsx`、`agent-team-save-feedback.tsx`——团队选择是快速切换器：默认只用单行用途、来源、主 Agent 与成员数建立选择依据，成员由「查看成员」在独立限高区域渐进披露；浮层同时受 Radix 可用高度、12px 视口安全边距和产品高度预算约束，已有会话只滚动候选目录而固定当前快照与说明。composer 更新提示按定义/运行配置/团队信息保持独立中性行，但任一操作都应用完整版本；历史头像 Popover 只展示 run 冻结事实并通过只读 Dialog 延迟读取完整 `AGENT.md`。浮层保持焦点回返，不显示内部摘要、路径、mtime、diff 或原生 `title` 提示。
