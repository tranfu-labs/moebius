...
- **Agent 表单卡片**：`src/console/agent-form-card.tsx` + `agent-form-model.ts`——Agent 需要用户拿主意时挂在 composer 上方的一份问答卡（产品事实源 `docs/product/pages/agent-form.md`）。卡片是 `rounded-xl border-line bg-card` 的三段式：头部是 20px 画像 + 成员名 + 右侧 `1/4`（`text-meta tabular-nums`）与一条逐题进度轨，中间是单题作答区，底部是导航行；头尾 `shrink-0`、中间 `min-h-0 flex-1 overflow-y-auto`，可用高度与宽度都由宿主给，组件不写 `vh`、不设宽度。进度轨每格是一个按钮，已答 `bg-accent`、未答 `bg-sel`，当前那格由 3px 升到 5px——**用高度而不是第三种颜色表达「在哪一题」**，否则「已答」和「当前」会抢同一个色位；整条轨是一个 tab stop（roving tabindex + 左右方向键），四格各占一个 tab stop 会让用户每题多按四下才够到答案。选项行是 `rounded-lg border-line` 的透明行，hover `bg-hover`，选中 `has-[:checked]:border-accent has-[:checked]:bg-sel`；控件是 `sr-only` 的原生 radio / checkbox，视觉落在相邻 `span` 上并由 `peer-checked` / `peer-focus-visible` 驱动（focus ring 必须走 `color-mix`，`ring-accent/40` 不生成规则）。**每个选项显式带 `aria-label` 合并 title 与 description**：说明本身就是判断依据，视觉隐藏的控件不能指望浏览器替你折进标签文本。「自己写」那一项由产品补在末尾、不属于表单定义，它没有自己的勾选控件——文字就是选中态，因此不能包进 `<label>`（多选下点输入框会连带切换 checkbox），改为整行 `onClick` 聚焦到自增高 `textarea`。前进 / 回退后焦点交给新一题的第一个控件，但**卡片出现时不抢焦点**。
