...
- **主会话目录轨**：`src/console/conversation-relay-rail.tsx` + `conversation-relay-rail-model.ts`——只固定在当前根会话正文左侧；收起为身份色短横线，展开为用户菱形、Agent 圆点与相邻行三次贝塞尔 Git graph 曲线。展开后的整条事件行是唯一节点命中区，预览 Popover 相对面板边缘保持固定间距；Agent 预览只显示可读成员名、时间与回复开头。目录滚轮 / 方向键只浏览，明确激活后才定位主时间线；连接不得跨省略区。
