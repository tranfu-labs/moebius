...
- **会话图片预览与 Lightbox**：`src/console/structured-attachments.tsx`、`conversation-image-dialog.tsx`——正常图片消息只显示 `128×96` 图片入口，不在卡片内重复文件名、格式或来源；这些信息只进入替代文字和辅助名称。Lightbox 复用 Radix Dialog 的焦点与关闭语义，但内容层使用接近全视口的透明画布和 `bg-black/90` 遮罩，只保留图片、左右切换和底部缩放控制；放大后才允许 Pointer Events 拖拽，切图时复位缩放与偏移。当前对话的图片顺序由宿主一次传入，组件不读取 HTTP、文件系统或缓存。
- **主会话目录轨**：`src/console/conversation-relay-rail.tsx` + `conversation-relay-rail-model.ts`——只固定在当前根会话面板左缘；收起为共同左端对齐的紧凑短横线，用户使用前景色、Agent 使用稳定身份色，当前阅读位置以更长更强的横线突出。展开时面板从同一左锚点向右打开，用户事件留在固定主干，Agent 圆点按成员首次出现顺序进入内容驱动的泳道；成员色分支从前一可见事件平滑分叉、纵向穿过该成员回复并在后一可见事件并回，省略区两侧必须收束断开。长会话高度预算取实测会话区高度约 70%，保留 24px 总纵向间隙并封顶 15 个 32px 行（480px）；矮窗口才在 32–20px 间压缩行高，禁止用 CSS 裁切替代模型窗口化。hover band 在轨迹下方、整条事件行在轨迹上方作为唯一节点命中区；视觉 band 相对命中区左右各内收 4px、上下各内收 2px，使用 `rounded-md`，不得贴住面板边界；当前节点使用身份色描边环。focused 外观下，面板使用 `--focused-side-surface` + `border-line`，hover band 使用 `--focused-interaction`，不得再用同一个 `--focused-sunken` 灰阶同时承担两层。省略行不选择事件，但属于轨道整体的 hover 保持区：关闭计时只由轨道整体离开触发，不得由内部事件行之间的移动触发。预览 Popover 相对整个面板边缘保持固定间距并沿事件行纵向跟随，只显示可读成员名、时间与有限行原文。外层测量壳在 `useLayoutEffect` 取得首个正高度之前不挂载 nav、Anchor 或 Popover；Anchor 是测量壳下与动画 nav 平级的稳定定位层，始终按最终展开 `stageTop` 与行高计算，首个可见几何因此就是最终几何，ResizeObserver 只驱动后续真实尺寸变化。目录滚轮 / 方向键只浏览，点击、Enter 或 Space 后才定位主时间线；不得使用全宽等距散点、逐相邻事件 S 线、居中膨胀或邻近项金字塔，reduced-motion 必须即时呈现等价静态信息。窄容器下消息列与输入框左缘固定让出收起态目录轨（56px = 12px 内缩 + 44px 收起视口），展开面板以悬浮覆盖层呈现（z-index 高于正文列），正文、标题、输入框位置不随展开变化。
...
- **过程步骤披露**：`src/console/process-trail.tsx`——运行中的记录在角色信息下先直接展开步骤列表，再显示当前活动或流式正文；终局消息收成内容宽度的「思考与工具调用 · N 步」disclosure，重新展开后仍使用同一组步骤行与单步详情，不另做历史简版。形式参考 Prompt Kit 的 [Steps](https://www.prompt-kit.com/docs/steps)、[Chain of Thought](https://www.prompt-kit.com/docs/chain-of-thought) 与 [Tool](https://www.prompt-kit.com/docs/tool) 的可观察组合关系，但保持本项目令牌与状态裁决：折叠态用 8px 状态圆点与 1px 连续轨迹保持轻量，14px 类型图标、标题和 12px mono 对象落在右侧内容列；展开时该行从透明容器升级为单个 `rounded-lg border-line` 工具面，行头、按类型命名的输入和结果连续相接，不得在行下再挂一张脱节卡片。过程区总 disclosure 与单步详情使用 `grid-template-rows` + opacity 的 150ms 令牌化过渡，从当前进度响应反向操作，不缩放、位移或弹跳内容，`prefers-reduced-motion` 下即时切换；这是用户于 2026-08-17 确认的局部空间动效例外。命令、路径、查询与调用参数使用带提示符的 `bg-sunken` 代码面，结果使用同形代码面；失败结果才使用 danger tint，完成态保持中性。详情仍先输入后输出，缺失记录原位说明，裁剪输出在底栏说明剩余行数并指向完整输出。失败色只标具体步骤，不改变整轮状态；运行节点与等待结果使用现有运行色、旋转图标及 reduced-motion 约束。过程区根部落在所属消息的 32px 正文列；终局重新展开时，步骤列表在摘要下再缩进 18px，用分组层级解释这些非齐头边界。
- **Agent 表单卡片**：`src/console/agent-form-card.tsx` + `agent-form-model.ts`——Agent 需要用户拿主意时挂在 composer 上方的一份问答卡（产品事实源 `docs/product/pages/agent-form.md`）。卡片是 `rounded-xl border-line bg-card` 的三段式：头部一行放 20px 画像 + 成员名，同行靠右是逐题进度格与 `1/4`（`text-meta tabular-nums`）；中间是单题作答区，底部是导航行；头尾 `shrink-0`、中间 `min-h-0 flex-1 overflow-y-auto`，可用高度与宽度都由宿主给，组件不写 `vh`、不设宽度。**进度不占整行、不横贯卡片**：满宽的进度轨会被读成页面在加载，而且在一张两三百像素高的卡里会比它索引的那道题还抢眼；收成 16px 一格的短横后，卡片里只剩「内容与操作」那一道横线（头部与正文之间靠留白分段）。每格是一个按钮，已答 `bg-accent`、未答 `bg-line`（`bg-sel` 在 16px 宽度下几乎看不见），当前那格由 3px 升到 5px——**用高度而不是第三种颜色表达「在哪一题」**，否则「已答」和「当前」会抢同一个色位；整条轨是一个 tab stop（roving tabindex + 左右方向键），四格各占一个 tab stop 会让用户每题多按四下才够到答案。问题标题用 `text-base font-semibold`（选项是 13px），它是这张卡的主线；成员名沿用全包的 `text-sm text-ink` 角色名写法，不为了压低它另造一档。导航行的上一步与下一步**并排在右下角**，不分居两端——两个动作是同一件事的两个方向，分开摆会让手在卡片两端来回跑。发送禁用时不写原因、不挂 `aria-describedby`、不加 tooltip：没有可发送的东西这件事，禁用态本身已经说完了。**卡片内部一条横线都不画**，头部 / 作答区 / 导航行三段靠留白分开——它已经是一块有边界的卡，里面再切两刀只会更碎。三种作答区共用同一种面：`rounded-lg border-line` 的透明行，自由输入题也不用 `bg-sunken` 灰底，否则同一张卡里选择题和问答题会长成两种东西。选项行的描边不是分割线，是「点整行都算」的命中区边界，不适用「横线能删就删」那条。**卡片高度过渡是第三条登记的空间动效例外**（2026-08-23 按产品决定加入）：题与题的作答区高低不同，而导航按钮就在刚点它的指针底下，尺寸一跳按钮就从手指底下跑了。实现与浮层例外同规矩——WAAPI、`--dur` + `--ease`、起点取**切题那一刻的实际高度**（写在 `commit` 里，不是 effect 里读终态），因此连点「下一步」是从当前进度继续而不是从上一次的终态重来；`prefers-reduced-motion` 与无 WAAPI 环境直接落终态。只动卡片自己的 `height`（配 `overflow-hidden`），**不对内容做位移、缩放或淡入**——内容换的是另一道题，不是同一块东西在移动。选中标记的勾/点走 `transition-opacity`，属于既有的颜色过渡默认档，不是例外。卡片由 `operator-console.tsx` 的底部 dock 组合，排在待发射区、附件草稿与输入框之前并共用同一条正文列宽——**表单是外来的，用户已经准备好的东西离输入框更近**；判定不可渲染时整块不渲染，界面上不出现任何解释写法问题的文案。选项行是 `rounded-lg border-line` 的透明行，hover `bg-hover`，选中 `has-[:checked]:border-accent has-[:checked]:bg-sel`；控件是 `sr-only` 的原生 radio / checkbox，视觉落在相邻 `span` 上并由 `peer-checked` / `peer-focus-visible` 驱动（focus ring 必须走 `color-mix`，`ring-accent/40` 不生成规则）。**每个选项显式带 `aria-label` 合并 title 与 description**：说明本身就是判断依据，视觉隐藏的控件不能指望浏览器替你折进标签文本。「自己写」那一项由产品补在末尾、不属于表单定义，它没有自己的勾选控件——文字就是选中态，因此不能包进 `<label>`（多选下点输入框会连带切换 checkbox），改为整行 `onClick` 聚焦到自增高 `textarea`。前进 / 回退后焦点交给新一题的第一个控件，但**卡片出现时不抢焦点**。
...
- **应用更新弹层**：`src/console/update-prompt-dialog.tsx`——更新就绪提醒与重启安装确认共享 `src/ui/dialog.tsx` 的 Radix 对话框、焦点限制和蒙版；提醒弹层的蒙版 / `Esc` / 空白点击统一归为“稍后提醒”，有运行任务的安装确认统一归为“继续工作”。动作使用既有 Button 变体，停止运行任务的安装动作使用 danger，弹层无阴影、无渐变，运行时决定与持久化不进入组件。
- **Claude 原生信任提示**：`src/console/claude-terminal-surface.tsx`——Claude Code 在首个任务前出现的已知原生提示由 runtime 在同一 PTY 自动确认；组件库只把原始字节按既有只读 terminal surface 显示，不渲染信任弹层、信任／拒绝按钮或可编辑输入，也不展示、读取或修改 Claude 信任记录。终端输出不能成为组件侧的决策依据，正常 Agent Markdown 仍由最终结果流程产生。
