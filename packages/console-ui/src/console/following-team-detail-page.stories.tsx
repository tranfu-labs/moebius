...
| 别人做的团队 | **空结果** | **加载中并保留结构** | **GitHub 不可用** | **无权限** | **超长文本** |
| 安装前预览 | 复用 **仓库格式不符**：没有合法团队内容时禁止安装 | **加载中** | **GitHub 不可用**／**安装失败** | **无权限** | **超长 AGENT.md** |
| 跟随上游团队详情 | **成员信息暂时无法读取** | **加载中** | **加载失败** | **上游不可访问**：本地团队仍可编辑和使用 | **超长文本** |

每个单元格均指向当前 Storybook 中的具体 Story 或明确复用项；无空白。

### 次要页面一致性自查

- **别人做的团队**：复用 Agent 团队首页的页面容器、标题、卡片、按钮与间距阶梯；窄窗语言过滤使用 PRD 指定的 Select，没有产生第二套筛选控件。
- **安装前预览**：正常、安装中、安装失败与长文本状态均复用只读 **AgentTeamDetail**；返回、标题、成员切换、Markdown 与操作层级继承既有详情页。
- **跟随上游团队详情**：全部状态复用可编辑 **AgentTeamDetail**；同语义字段、成员选择、运行配置、头像选择、AGENT.md 与“保存”主操作只有一个组件变体。
...
- **操作层级**：安装前预览只有“安装”一个主操作，跟随上游详情只有“保存”一个主操作；返回、打开仓库、重试、撤销、更多操作使用 outline／ghost 或文本链接层级。
...
1. **【呈现层】别人做的团队／结果区**：PRD 字符图把结果画在一块带分隔线的列表内；设计稿改为沿用 Agent 团队首页的独立卡片行。理由：复用已确立的团队浏览结构，同时让整卡预览与仓库外链保持两个独立命中目标。
