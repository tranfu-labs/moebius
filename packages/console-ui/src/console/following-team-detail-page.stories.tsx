...
| GitHub 团队详情 | **成员信息暂时无法读取** | **加载中** | **加载失败** | 复用 **加载失败** | **超长文本** |
...
- **GitHub 团队详情**：全部状态复用可编辑 **AgentTeamDetail**；同语义字段、成员选择、运行配置、头像选择、AGENT.md 与“保存”主操作只有一个组件变体。
...
- **操作层级**：安装前预览只有“安装”一个主操作，GitHub 来源团队详情只有“保存”一个主操作；返回、打开仓库、重试、更多操作使用 outline／ghost 或文本链接层级。
- **响应式**：390px 下发现页语言过滤改用 Select；结果元信息纵向换行；成员条保持单行横向滚动；安装区固定在视口底部，页面不产生横向溢出。
- **集成边界**：Story 只表达搜索、打开仓库、安装、保存和导航意图，不访问 GitHub、gh 登录态、IPC、磁盘或用户数据。
...
- **信息项落位**：Agent 团队首页的「找现成团队／新建团队」常驻入口、新建菜单两条路径、GitHub 来源／本地分组与来源仓库身份已落位。
- **信息项落位**：发现页的搜索、语言、登录范围、名称、用途、仓库、star、更新时间、语言与私有标记已落位。
- **信息项落位**：预览页的仓库身份、主 Agent、成员总数、逐名推荐配置、完整 AGENT.md、三项安装后果及安装状态已落位。
- **信息项落位**：GitHub 来源团队详情的来源仓库、成员、运行配置、AGENT.md、管理操作与单一保存已落位。
...
export const GithubSourceTeam: Story = {
  name: "GitHub 来源团队",
...
export const EmptyMembers: Story = {
  name: "成员信息暂时无法读取",
  args: { members: [], selectedMemberSlug: undefined },
};

export const Loading: Story = {
  name: "加载中",
  args: { contentState: "loading" },
};

export const LoadFailed: Story = {
  name: "加载失败",
  args: { contentState: "error", onRetryLoad: () => undefined },
};
