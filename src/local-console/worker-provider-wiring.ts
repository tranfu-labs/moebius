...
export function createLocalWorkerProviderPorts(input: {
  ...
  return {
    ...
    onVisibleAgentMarkdown: (workerInput, text) => {
      const target = decideWorkerActiveRunTarget(input.activeRun(workerInput.runId), workerInput.sessionId);
    ...
    setActiveExternalSessionId: (sessionId, runId, externalSessionId) => {
      const target = decideWorkerActiveRunTarget(input.activeRun(runId), sessionId);
