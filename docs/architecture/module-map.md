...
当前仓库已提供 TypeScript 运行时代码与可选 Electron 桌面壳；`agents/` 仍只作为 Markdown 素材模块记录，不承担运行时状态。`pnpm check:boundaries` 以 TypeScript AST 检查下列 `[IB:*]` import 边界，并校验每条禁止项均登记为唯一 `[IB:*]` 或带原因的 `[NI:*]`；完整与 scope 测试会先执行该门禁。
...
- 禁止依赖：MUST NOT 给 observer 增加写接口或 runner 控制能力。[NI:desktop-observer-readonly]（非 import：属于 IPC 与运行时能力） ；MUST NOT 复制 runner / observer / ledger / local-console 业务规则。[NI:desktop-no-business-rule-copy]（非 import：语义复制不能由依赖图判定）；MUST NOT 把用户配置或 agent 素材写回应用资源目录。[NI:desktop-no-resource-writeback]（非 import：同一文件系统 adapter 同时承担合法读取与受限写入）；MUST NOT 用 shell 字符串拼接外部输入。[NI:desktop-no-shell-concatenation]（非 import：需检查调用参数的数据流）；MUST NOT 在同一实例内派生多个 runner。[NI:desktop-single-runner]（非 import：属于运行时实例计数）；MUST NOT 在 runner child 已显式选择 GitHub mode 时再启动第二个 local console server。[NI:desktop-no-dual-runtime]（非 import：属于启动时状态组合）；Agent 团队 UI MUST NOT 另行复制结构有效性规则，只消费 store/IPC 给出的状态、可用性与 issue code。[NI:desktop-team-validation-single-source]（非 import：业务规则复制不能由依赖图判定）；移除失效记录 MUST NOT 删除、移动或修改团队目录内容。[NI:desktop-repair-record-only]（非 import：需验证文件系统副作用）
...
- 禁止依赖：MUST NOT 反向 import `src/runner*`、`src/observer*`、`src/local-console*`、`src/goal-ledger*` 或任何 `.state` adapter。[IB:console-ui-no-runtime-internals]；MUST NOT 调用 `gh` / `codex` / shell。[IB:console-ui-no-side-effect-adapters]；MUST NOT 把业务事实复制进组件层。[NI:console-ui-no-business-fact-copy]（非 import：语义复制不能由依赖图判定）真实 renderer app、IPC 与 local console API 对接属于 desktop-shell。
...
- Provider 原生过程记录验收边界：`scripts/acceptance/provider-native-process-traces.ts` 用真实 Claude/Kimi CLI 生成原生 session 文件，再以临时 Moebius 数据根启动真实 Electron，断言过程事件投影、resume 多 attempt 同源、重启恢复和原生文件删除降级；脚本只移动并恢复自身新建的 Claude transcript，不处理其他会话。
- Kimi 空响应验收边界：`scripts/acceptance/kimi-empty-response.ts` 只在系统临时数据根冻结单成员 Kimi 团队，以真实 Electron 主对话和真实 Kimi ACP CLI 复现空 `end_turn`，断言安全失败、同一 canonical session resume、无 attempt execution link/空回复/cursor、重启保持及过程记录不可用；脚本只把 bounded 脱敏 evidence 写到系统临时目录，额度状态不再复现时 fail closed 报告前提不成立。
...
- 入口：`src/local-console/server.ts`、`src/local-console/runtime.ts`、`src/local-console/session-reference-text.ts`、`src/local-console/user-message-routing.ts`、`src/local-console/run-activity.ts`、`src/local-console/execution-context.ts`、`src/local-console/execution-driver.ts`、`src/local-console/codex-resume.ts`、`src/local-console/codex-thread-link.ts`、`src/local-console/codex-rollout.ts`、`src/local-console/provider-process-trace.ts`、`src/local-console/process-history.ts`、`src/local-console/process-event-projector.ts`、`src/local-console/prompt.ts`、`src/local-console/store.ts`、`src/local-console/attachments.ts`、`src/local-console/output-tail.ts`；`user-message-routing.ts` 只依赖共享 mention 纯函数与调用方传入的成员名单，不依赖 UI、SQLite、provider 或 shell；`provider-process-trace.ts` 只负责按 execution link 分派 Codex rollout、Claude transcript 与 Kimi main wire 的可信定位和 provider-native context，事件语义留在 projector，attempt 聚合与游标留在 process-history；三家共用的 realpath / regular-file / device / inode / monotonic-size 与 JSONL previous/append 读取在 `src/trusted-jsonl.ts`，不得在 provider adapter 复制。Codex 适配与最低 CLI 版本契约在 `src/codex.ts`、`src/codex-cli-version.ts`、`src/codex-rollout.ts`（local 路径仅兼容 re-export），Claude 适配与最低版本契约在 `src/claude.ts`、`src/claude-cli-version.ts`、`src/claude-executable.ts`，Kimi ACP 适配及其受管运行目录隔离在 `src/kimi.ts`、`src/kimi-runtime-home.ts`。
...
- 禁止依赖：MUST NOT 调用 GitHub / artifact publisher / GitHub CEO orchestration executor。[IB:local-console-no-github-runtime]；`control-dispatch.ts` MUST NOT 经直接或运行时传递依赖到达文件系统、SQLite、provider 或执行 driver adapter。[IB:local-control-planner-pure-closure]；`run-invocation-plan.ts` MUST NOT 经直接或运行时传递依赖到达文件系统、SQLite、provider 或执行 driver adapter。[IB:local-invocation-planner-pure-closure]；MUST NOT 修改 conversation、trigger、stage、goal-ledger 或 GitHub issue runner 的业务规则。[NI:local-console-no-upstream-rule-mutation]（非 import：规则修改与复制不能由依赖图判定）；MUST NOT 启动 GitHub heartbeat、读取 GitHub intake 或把 local session 数据镜像到 `.state/github-runner.sqlite`。[NI:local-console-no-github-runtime-state]（非 import：属于运行时启动与数据流副作用）；本地 child session orchestration 只允许复用纯 parser 并映射为 local child session、`sessions.parent_session_id` 和桌面子会话呈现，不得反向调用 GitHub child issue 编排。[NI:local-console-child-session-local-only]（非 import：纯 parser 与 executor 共处上游模块，需行为验证）；本地 descriptor 的任务检查可选，legacy `acceptanceStatements` 只兼容显示为“任务检查参考”，不得恢复 formal acceptance scope。[NI:local-console-no-formal-acceptance-restore]（非 import：属于业务语义）dead-letter/recovery 仍仅限本地 SQLite 可见收敛。
...
- 禁止依赖：MUST NOT 依赖运行时状态文件、GitHub token 或本地脚本输出。[NI:agents-static-material-only]（非 import：Markdown 素材没有 TypeScript import graph）
...
- 下游：无真实外部操作。
- 禁止依赖：MUST NOT 调用 `gh` / `codex` / 文件系统。[IB:stages-no-side-effect-adapters]；MUST NOT 依赖 runner 状态。[NI:stages-no-runner-state]（非 import：状态数据可经普通对象传入）；MUST NOT 把 stage 白名单复制到触发器或 CEO 模块里各自维护。[NI:stages-single-whitelist]（非 import：常量复制不能由依赖图判定）
...
- 禁止依赖：MUST NOT 调用 GitHub。[IB:ceo-format-no-github-adapter]；MUST NOT 更新 `.state/*`。[NI:ceo-format-no-state-write]（非 import：该模块合法读取文件，写入需行为验证）；MUST NOT 复用 dev / 其他 role thread。[NI:ceo-format-own-thread]（非 import：属于 provider session 选择）；MUST NOT 把事故处理规则硬编码到 TypeScript 里。[NI:ceo-format-script-source]（非 import：内容来源不能由依赖图判定）；MUST NOT 因 CEO 失败阻断原 agent 评论发布。[NI:ceo-format-fail-open]（非 import：属于调用顺序与失败语义）
...
- 禁止依赖：MUST NOT 调用 GitHub / Codex。[IB:ceo-scripts-no-provider-adapters]；MUST NOT 写文件。[NI:ceo-scripts-readonly-files]（非 import：模块合法读取文件）；MUST NOT 把剧本文件当作可 mention agent。[NI:ceo-scripts-not-agents]（非 import：属于素材分类语义）
...
- 禁止依赖：MUST NOT 调用 GitHub / Codex / 文件系统 / shell。[IB:ceo-orchestration-no-side-effect-adapters]；MUST NOT 直接持久化状态。[NI:ceo-orchestration-no-persistence]（非 import：普通注入函数也可产生持久化副作用）
...
- 禁止依赖：MUST NOT 调用 `gh` / `codex` / 文件系统。[IB:triggers-no-side-effect-adapters]；MUST NOT 把 issue 内容拼成 shell 命令。[NI:triggers-no-shell-content]（非 import：需追踪外部输入数据流）；MUST NOT 把 stage / CEO guardrail 业务规则写进 trigger。[NI:triggers-no-upstream-rule-copy]（非 import：语义复制不能由依赖图判定）
...
- 禁止依赖：MUST NOT 执行 issue body/comment 中声明的任意脚本路径。[NI:agent-prescripts-trusted-registry-only]（非 import：需验证脚本来源数据流）；MUST NOT 用 shell 拼接外部输入。[NI:agent-prescripts-no-shell-concatenation]（非 import：需检查调用参数）；MUST NOT 把运行状态写入 `agents/`。[NI:agent-prescripts-no-agent-state]（非 import：需验证文件系统目标）
...
- 下游：无真实外部操作。
- 禁止依赖：MUST NOT 调用 `gh` / `codex` / 文件系统。[IB:response-intake-no-side-effect-adapters]；MUST NOT 读取 agent 文件或构造 prompt。[NI:response-intake-no-agent-prompt]（非 import：prompt 可在本模块内构造，需行为审查）；MUST NOT 把 issue 内容拼成 shell 命令。[NI:response-intake-no-shell-content]（非 import：需追踪外部输入数据流）
...
- 禁止依赖：MUST NOT 调用 `gh` / `codex` / 文件系统。[IB:driver-pool-no-side-effect-adapters]；MUST NOT 读取或写入 `.state/*`。[NI:driver-pool-no-state-io]（非 import：文件系统依赖已拦截但注入回调仍需行为验证）；MUST NOT 引入 GitHub issue domain types、trigger 规则或 prompt 构造。[NI:driver-pool-domain-neutral]（非 import：类型与语义边界不能完全由路径表达）
...
- 下游：TOML parser 依赖。
- 禁止依赖：MUST NOT 调用 `gh` / `codex`。[IB:local-config-no-provider-adapters]；MUST NOT 读取 GitHub issue 内容。[NI:local-config-no-issue-content]（非 import：内容可经普通字符串传入）；MUST NOT 保存 token 或运行状态。[NI:local-config-no-sensitive-state]（非 import：需验证持久化内容）
...
- 禁止依赖：MUST NOT 被 `runner.ts`、scanner、dispatcher、state persister 或任何 GitHub / Codex 主链路模块依赖。[IB:observer-no-main-chain-dependents]；MUST NOT 调用 `gh` / `codex` / artifact publisher。[IB:observer-no-write-adapters]；MUST NOT 写 `.state/*`、run manifest、release asset、worktree 文件或运行时状态。[NI:observer-no-runtime-write]（非 import：observer 合法使用文件系统读取）；MUST NOT 提供重跑、ack、发布、同步等操作能力。[NI:observer-readonly-api]（非 import：属于 HTTP API 与运行时能力）
...
- 禁止依赖：MUST NOT 依赖 `agents/` 作为运行状态。[NI:runner-no-agent-state]（非 import：需验证持久化目标）；MUST NOT 直接拼接 issue 内容为 shell 命令。[NI:runner-no-shell-content]（非 import：需追踪外部输入数据流）；MUST NOT 在 codex 失败时发评论。[NI:runner-no-comment-on-codex-failure]（非 import：属于失败路径与调用顺序）；MUST NOT 让 `src/goal-ledger.ts`、`src/conversation.ts`、`src/github-response-intake.ts`、`src/driver-pool.ts`、`src/triggers/*` 或 `src/observer/*` 反向 import `src/runner/*`。[IB:runner-no-reverse-dependencies]；MUST NOT 把 acceptance、orchestration、mention 或 route judgement 的业务事实复制进 runner 子模块形成多事实源。[NI:runner-no-domain-fact-copy]（非 import：语义复制不能由依赖图判定）
...
- 禁止依赖：MUST NOT 调用 `codex`。[IB:scanner-no-codex-adapter]；MUST NOT 触发 issue 处理或写状态文件（状态变更只通过注入的 applyState 应用）。[NI:scanner-discovery-only]（非 import：注入回调的调用语义不能由依赖图判定）
...
- 禁止依赖：MUST NOT 调用 `gh` / `codex` / 文件系统。[IB:dispatcher-no-side-effect-adapters]；MUST NOT 理解 trigger、prompt 或 Codex 参数（job 内容由注入的 runJob 承载）。[NI:dispatcher-job-opaque]（非 import：数据 shape 与业务理解不能由依赖图判定）
...
- 禁止依赖：MUST NOT 理解 GitHub / Codex / trigger 语义。[IB:state-persister-no-domain-adapters]；MUST NOT 主动读文件（初始状态由组装方注入）。[NI:state-persister-no-read]（非 import：模块通过注入 writer 合法持久化，读写方向需行为验证）
...
- 下游：无真实外部操作。
- 禁止依赖：MUST NOT 调用 `gh` / `codex` / 文件系统。[IB:conversation-no-side-effect-adapters]；MUST NOT 把 issue 内容拼成 shell 命令。[NI:conversation-no-shell-content]（非 import：需追踪外部输入数据流）
...
- 禁止依赖：MUST NOT 调用 `gh` / `codex` / `fetch` / 文件系统。[IB:issue-media-no-side-effect-adapters]；MUST NOT 把 issue 内容拼成 shell 命令。[NI:issue-media-no-shell-content]（非 import：需追踪外部输入数据流）；MUST NOT 自行推进 role thread 或 intake state。[NI:issue-media-no-state-advance]（非 import：状态变更可经注入回调发生）
...
- 禁止依赖：MUST NOT 调用 `codex`。[IB:media-assets-no-codex-adapter]；MUST NOT 写入 `agents/`、`.state/` 或目标 worktree 的输入媒体。[NI:media-assets-bounded-write-targets]（非 import：该 adapter 合法使用文件系统写 runDir）；MUST NOT 提交生成产物到业务仓库。[NI:media-assets-no-repository-commit]（非 import：属于外部命令与副作用目标）；MUST NOT 把外部 URL 或文件名拼成 shell 命令。[NI:media-assets-no-shell-content]（非 import：需追踪外部输入数据流）
...
- 禁止依赖：MUST NOT 调用 `gh` / `codex` / 文件系统。[IB:conversation-interrupt-no-side-effect-adapters]；MUST NOT 知道具体 driver 类型。[NI:conversation-interrupt-driver-neutral]（非 import：类型可结构化兼容，需 API 审查）；MUST NOT 解析 issue 内容或构造 prompt。[NI:conversation-interrupt-no-domain-content]（非 import：内容处理语义不能由依赖图判定）
...
- 禁止依赖：MUST NOT 执行来自 issue body / comment 的任意命令。[NI:script-executor-trusted-command-only]（非 import：需追踪命令来源与 argv）；MUST NOT 在日志中输出敏感配置。[NI:script-executor-no-sensitive-log]（非 import：需验证运行时日志内容）
...
- 禁止依赖：`src/goal-ledger.ts` MUST NOT 调用 GitHub / Codex / 文件系统 / shell。[IB:goal-ledger-no-side-effect-adapters]；`src/goal-ledger-state.ts` MUST NOT 调用 GitHub / Codex 或执行 issue 内容。[IB:goal-ledger-state-no-provider-adapters]；MUST NOT 存放在 `agents/`。[NI:goal-ledger-no-agent-storage]（非 import：需验证持久化路径）；MUST NOT 把 run manifest 当成目标账本唯一事实源。[NI:goal-ledger-no-manifest-single-source]（非 import：属于事实源语义）；MUST NOT 让 observer 成为账本写入口或复用会全局 fail-closed 的执行期 projection/assert 路径。[NI:goal-ledger-no-observer-write-path]（非 import：observer 合法只读 import 账本纯函数）；MUST NOT 被 runner 主链路隐式依赖。[NI:goal-ledger-explicit-runner-boundary]（非 import：runner 存在合法显式调用，需装配审查）
...
- 禁止依赖：MUST NOT 存放在 `agents/`。[NI:role-thread-no-agent-storage]（非 import：需验证持久化路径）；MUST NOT 存 GitHub token、prompt 全文或 codex 执行日志。[NI:role-thread-bounded-content]（非 import：需验证持久化内容）
...
- 禁止依赖：MUST NOT 存放在 `agents/`。[NI:agent-context-no-agent-storage]（非 import：需验证持久化路径）；MUST NOT 存 GitHub token、prompt 全文或 codex 执行日志。[NI:agent-context-bounded-content]（非 import：需验证持久化内容）
...
- 禁止依赖：MUST NOT 存放在 `agents/`。[NI:intake-state-no-agent-storage]（非 import：需验证持久化路径）；MUST NOT 存 GitHub token、prompt 全文、comment 正文或 codex 执行日志。[NI:intake-state-bounded-content]（非 import：需验证持久化内容）
...
- 上游：`github-issue-runner`
- 下游：本机 `gh` CLI。
- 禁止依赖：MUST NOT 在命令参数中拼接 shell 字符串。[NI:github-client-no-shell-concatenation]（非 import：需检查 argv 构造）；评论正文和 issue body MUST 通过 `--body-file -` 从 stdin 传入。[NI:github-client-body-via-stdin]（非 import：属于进程调用参数与 stdin 行为）；reaction target 与 content MUST 来自受控枚举 / adapter shape。[NI:github-client-controlled-reaction]（非 import：需验证运行时输入校验）；artifact asset 名称 MUST 来自已校验 / 清洗后的本地 artifact 数据。[NI:github-client-sanitized-assets]（非 import：需验证数据流）；可见写操作 MUST 保持无自动重试。[NI:github-client-no-write-retry]（非 import：属于运行时重试策略）
