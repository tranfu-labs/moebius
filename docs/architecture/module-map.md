...
### provider-adapters
- 职责边界：`src/codex.ts`、`src/claude.ts`、`src/claude-tui-transport.ts`、`src/claude-tui-node-pty.ts`、`src/claude-tui-lifecycle.ts`、`src/claude-tui-transcript.ts`、`src/claude-tui-workspace-trust.ts`、`src/kimi.ts` 与 `src/pi-execution-adapter.ts` 把三家 CLI 和 Pi API 映射为统一执行契约。可执行文件解析、版本探测、Claude PTY generation/idle 时序、私有 loopback lifecycle capability、Pi Host framing、原生 transcript/wire/安全 trace 读取与 canonical session 校验分别留在窄 adapter；Claude 每轮输入前的可信 transcript 记录边界只用于 Stop 后选择本轮 final/usage，不能驱动 lifecycle。Claude PTY transport、首次任务前的原生工作区信任提示识别和 lifecycle receiver 不读取 provider 内容或决定路由。对已知原生提示，adapter 只向同一 PTY 写入一次 Enter，再只等待正常输入提示，随后写入保留任务；未识别、重复或首任务后的 terminal text 不得追加 Enter，并且不产生 lifecycle、正文或 usage 事实；首个任务写入后即停止检测。Pi Host 只接受单次 invocation 的长度前缀 stdin/stdout 帧，Key 不进入 argv/env；Claude 的交互 PTY 由其 canonical session 在 idle 前持有，稳定 MCP relay 只读取路径而由 `claude-tui-managed-process-lease.ts` 在每个 active turn 轮换 capability，不能持有永久 provider-run capability。业务 dispatch、重试与终局文案不进入 provider adapter。
...
![claude-tui-auto-workspace-trust](claude-tui-auto-workspace-trust.svg)
