...
- Sidebar chat 事实边界：sidebar chat 仍是普通根 session；JSONL 保存来源导航、入口策略和有序静态文本片段，SQLite 保存可重建投影、归档/恢复与方案闸门状态。`session-analysis-gate.ts` 只解析受信任的末尾控制 envelope；runtime 在确认前把 Codex/Claude/Kimi 强制投影为只读，并仅为匹配当前方案版本的同一 Agent resume 建立一次性写入 lease。`session-reference-text.ts` 按显式 `message` / `conversation` scope 只格式化可信 session fact log 与 provider link：消息级精确匹配 run 或写「未建立」，对话级只返回记录路径，不读取来源记录或扩张权限。
