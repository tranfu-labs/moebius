...
自上而下：交互层（GitHub 及对外功能，含本地只读观察页与桌面状态页）→ 运维层（仓库、issue、Codex 的规模与节奏管理）→ 目标编排层（跨 issue 的目标入账、目标账本、子 issue 派生与验收回流）→ issue 内处理（单个 issue 的处理能力）→ 大模型执行层（本地会话支持 Codex/Kimi，GitHub runner 使用 Codex）→ 资产及其维护。图中块名按业务能力命名，不与具体文件绑定；文件级职责见下方各模块条目。
...
- Sidebar chat 编排边界：`desktop/src/console-page/presentation-route.ts` 分离左侧选中会话、主内容承载会话、右侧普通会话与标签宿主，`sidebar-conversation-drafts.ts` 和集中式标签文档保存未发送草稿及跨宿主标签现场；消息级与对话级分析 intent 复用同一草稿和标签模型，非当前对话先准备来源视图、可信片段、草稿与标签，全部成功后才原子提交组合路由。renderer 只编排 local-console 的普通 session 与 `console-ui` 受控组件，不复制消息、运行或权限规则。`browser-fetch.ts` 是原生 `window.fetch` 的 receiver-safe browser adapter。官方「通用助手」仍由既有 `team-seed.ts` / `team-store.ts` 边界登记和恢复，不进入 renderer 业务规则。
...
- 入口：`desktop/src/main.ts`；runner 子进程入口 `desktop/src/runner-child.ts`；preload `desktop/src/preload.ts`；操作台 renderer `desktop/src/console-page/*`；辅助状态页 `desktop/src/status-page/*`；纯逻辑模块 `desktop/src/data-root.ts`、`desktop/src/shell-path.ts`、`desktop/src/env-doctor.ts`、`desktop/src/runner-supervisor.ts`、`desktop/src/updater.ts`。preload 按 browser 边界构建，只运行时依赖 `electron`；IPC channel 与 renderer DTO 来自 `team-*-contract.ts`、`ai-team-builder/contract.ts`、`onboarding/contract.ts` 等无 Node runtime 依赖的纯契约，main IPC adapter 才能依赖文件系统、配置与 CLI service。首次引导的双 CLI 版本/能力收敛与单调 revision 在 `onboarding/cli-readiness*.ts`，受信任安装 registry、并发任务、进程回收和安全状态投影在 `onboarding/cli-installer*.ts`；renderer 只能通过 onboarding 窄契约提交 CLI 枚举。AI 建队状态机与持久化留在 `ai-team-builder/index.ts`，每个 draft 首次创建 provider session，后续 submit / adjust / repair / retry 只 resume 原 ID，失败保留 ID 且不重建；Codex/Kimi 的冻结 execution profile、统一 driver 边界和只读 spawner 分别收敛在 `ai-team-builder/execution-profile.ts`、`driver.ts`、`codex-spawner.ts` 与 `kimi-spawner.ts`。Agent 团队实现模块收敛在 `desktop/src/team-*.ts`：打包官方内容登记与旧 marker 迁移在 `team-seed.ts`，结构与有效性判定在 `team-model.ts`，首次引导编排的版本化读写、局部降级与旧内嵌数据迁移在 `team-onboarding-orchestration.ts`，受校验的团队磁盘读写在 `team-store.ts`，用户团队稳定记录、团队定义摘要与重定位身份校验在 `team-record-store.ts`，运行配置纯规则在 `team-execution-profile.ts`，运行配置与官方基线持久化在 `team-management-store.ts`，官方三方状态与迁移规划在 `team-official-management.ts`，保护副本与更新提交在 `team-official-update.ts`，Codex/Kimi 能力探测在 `execution-capabilities.ts` 且只由 onboarding、AI 建队或显式诊断流程消费，轻量 Codex doctor 只在用户打开现有状态页时运行；团队列表 / 成员读写 / 复制 / 更新的窄 IPC 在 `team-ipc.ts` 且只处理静态运行配置，重新定位与仅移除记录的 IPC 独立收敛于 `team-repair-ipc.ts`，成员 `AGENT.md` 外部修改探测与冲突判定在 `team-external-change.ts`，在平台文件管理器中打开与用户团队移到废纸篓在 `team-file-manager.ts`，上一次成功创建会话所用团队的记录与回退预选在 `team-conversation-preference.ts`，会话绑定团队的实时健康度、严格成员名单与有效执行配置解析在 `team-runtime-binding.ts`。`team.json` 只承载团队核心，`onboarding-orchestration.json` 只供首次引导演示；后者不得进入团队健康度、身份指纹、会话 roster 或 prompt。用户团队位置一律经记录解析；官方来源团队仍按 `.system/<id>` 解析且来源身份与删除能力受保护。
...
### console-ui
- Markdown 边界：共享 `MarkdownMessage` / Streamdown 呈现静态用户与 Agent 正文及单条活动 run Markdown，统一承载 GFM、CJK、代码、数学、Mermaid 与 URL/HTML 收紧；显式绝对路径链接只投影为应用内文件引用，已知成员 mention 只按会话名单投影为团队详情入口，两者都通过宿主回调交出意图，普通裸路径继续脱敏；系统事实仍属于结构化组件。
- Sidebar chat UI 边界：消息菜单和左侧栏对话菜单只发出来源对象明确的受控 intent，右侧栏的普通新会话与已有会话继续复用 `NewConversationPage` / `OperatorConsole` 生产组合；文本片段、候选问题、会话搜索、团队重名标签和 conversation tab 只提供通用受控 props 与纯状态转换。`Page/Console/SessionAnalysis` 使用确定性 fixture 展示两类真实生产菜单、禁用原因、组合路由和片段差异，不接 IPC、持久化或 provider。
...
- 入口：`src/local-console/server.ts`、`src/local-console/runtime.ts`、`src/local-console/session-reference-text.ts`、`src/local-console/run-activity.ts`、`src/local-console/execution-context.ts`、`src/local-console/execution-driver.ts`、`src/local-console/codex-resume.ts`、`src/local-console/codex-thread-link.ts`、`src/local-console/codex-rollout.ts`、`src/local-console/process-history.ts`、`src/local-console/process-event-projector.ts`、`src/local-console/prompt.ts`、`src/local-console/store.ts`、`src/local-console/attachments.ts`、`src/local-console/output-tail.ts`；Codex 适配、最低 CLI 版本契约与共享 rollout identity resolver 在 `src/codex.ts`、`src/codex-cli-version.ts`、`src/codex-rollout.ts`（local 路径仅兼容 re-export），Kimi ACP 适配及其受管运行目录隔离在 `src/kimi.ts`、`src/kimi-runtime-home.ts`。
