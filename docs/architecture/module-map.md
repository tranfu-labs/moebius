...
- Sidebar chat 编排边界：`desktop/src/console-page/presentation-route.ts` 分离左侧选中会话、主内容承载会话、右侧普通会话与标签宿主，`sidebar-conversation-drafts.ts` 和集中式标签文档保存未发送草稿及跨宿主标签现场；renderer 只编排 local-console 的普通 session 与 `console-ui` 受控组件，不复制消息、运行或权限规则。`browser-fetch.ts` 是原生 `window.fetch` 的 receiver-safe browser adapter。官方「通用助手」仍由既有 `team-seed.ts` / `team-store.ts` 边界登记和恢复，不进入 renderer 业务规则。
...
- Sidebar chat UI 边界：右侧栏的普通新会话与已有会话直接复用 `NewConversationPage` / `OperatorConsole` 生产组合；文本片段、候选问题、会话搜索、团队重名标签和 conversation tab 只提供通用受控 props 与纯状态转换。`Page/Console/SessionAnalysis` 使用确定性 fixture 展示这些生产组合，不接 IPC、持久化或 provider。
...
- Sidebar chat 事实边界：sidebar chat 仍是普通根 session；JSONL 保存来源导航、入口策略和有序静态文本片段，SQLite 保存可重建投影、归档/恢复与方案闸门状态。`session-analysis-gate.ts` 只解析受信任的末尾控制 envelope；runtime 在确认前把 Codex/Kimi 强制投影为只读，并仅为匹配当前方案版本的同一 Agent resume 建立一次性写入 lease。静态引用端点只格式化可信 session/run 标识，不读取来源记录或扩张权限。
