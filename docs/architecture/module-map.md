...
### local-console
- 职责边界：本地操作台提供 loopback HTTP、SQLite 可变状态与 JSONL 会话事实；负责项目/会话、完整团队快照、三类变化检测、effective/candidate/pending 与持久化 apply intent、dispatch 快照代次、run 审计投影、四引擎执行配置、消息 FIFO、主 Agent/成员接力、provider canonical session、执行代际迁移/结束、恢复/中断/失败、附件、运行过程、文件引用，以及当前应用生命周期内的会话级托管进程。状态查询在目录、团队连续性、关注状态、运行活动与轮次状态全部汇合后生成不可变的会话状态点；侧栏、项目聚合与 Dock 只消费该投影，投影不落盘。主 Agent 派工世代由 `handoff-dispatch-runtime.ts`（application）编排、`control-dispatch.ts`（domain）纯决策：每次派工持久化递增 generation 事实，同角色新派工覆盖旧派工（未启动取消、晚到结果不交棒），用户直达不参与世代。SQLite 只保存可变版本流转，run 当时的完整团队/profile 与外部启动证据追加到 JSONL；窄 API 只按 session/run 暴露审计、显式 Markdown 与按 attempt 的 Claude 终端诊断读取。`claude-terminal-trace.ts` 只保存 bounded、按序 raw PTY bytes/delta 的 active projection；`claude-terminal-trace-store.ts` 将对应 Claude attempt 的诊断追加到 runDir 元数据文件，既不成为 JSONL 正文，也不参与 transcript/lifecycle 推断。Pi 安全过程记录持久化在数据根并以事实中的可信路径关联；Provider Key、ciphertext 与原始请求响应不得进入会话事实。普通 provider run 由单轮 stdio MCP bridge 进入 supervisor；Claude 持久 TUI 改由同一 relay 的 lease-file 模式在每个 active turn 申请并撤销 capability，再由认证 launchd ownership job 持有。Claude 原生工作区信任提示只由 provider adapter 在首个任务写入前识别并一次性确认；local-console 不投影 `workspaceTrust` 状态、不提供 trust-decision route，且不读取或修改 Claude 信任记录。注册表不进入会话事实，重启只精确清理而不恢复或重放命令。`runtime.ts` 仅保留窄 composition root；决策规则在纯 `*plan.ts` / `runtime-domain.ts`，时序在 application runtime，fs/SQLite/provider/HTTP/launchd 在 adapter。新会话自动标题生成由 `session-title-plan.ts`（domain）判定、`session-title-runtime.ts`（application）编排，触发面覆盖 create + initialMessage 与 submit 两条首条消息落库入口，共享同一 runtime 实例与进程内在途守卫。
...
### provider-adapters
- 职责边界：`src/codex.ts`、`src/claude.ts`、`src/claude-tui-transport.ts`、`src/claude-tui-node-pty.ts`、`src/claude-tui-lifecycle.ts`、`src/claude-tui-transcript.ts`、`src/claude-tui-transcript-follower.ts`、`src/claude-tui-native-prompt.ts`、`src/kimi.ts` 与 `src/pi-execution-adapter.ts` 把三家 CLI 和 Pi API 映射为统一执行契约。可执行文件解析、版本探测、Claude PTY generation/idle 时序、私有 loopback lifecycle capability、Pi Host framing、原生 transcript/wire/安全 trace 读取与 canonical session 校验分别留在窄 adapter；Claude 每轮输入前的可信 transcript 记录边界只用于 Stop 后选择本轮 final/usage，不能驱动 lifecycle。Claude PTY transport、首次任务前的原生确认门和 lifecycle receiver 不读取 provider 内容或决定路由。对已知原生提示，adapter 只向同一 PTY 写入一次对应 native key，再只等待正常输入提示，随后写入保留任务；未知候选项只发布一次性待决事实并由后续窄通道选择，无法抽取候选项时进入稳定安全失败；重复或首任务后的 terminal text 不得追加按键，并且不产生 lifecycle、正文或 usage 事实；首个任务写入后即停止检测。Claude transcript follower 只读已验证 JSONL 的完整追加记录，使用本轮边界后的 byte cursor，按 bounded polling 运行，停止或身份异常时释放并静默降级。Pi Host 只接受单次 invocation 的长度前缀 stdin/stdout 帧，Key 不进入 argv/env；Claude 的交互 PTY 由其 canonical session 在 idle 前持有，稳定 MCP relay 只读取路径而由 `claude-tui-managed-process-lease.ts` 在每个 active turn 轮换 capability，不能持有永久 provider-run capability。业务 dispatch、重试与终局文案不进入 provider adapter。
- 入口：`src/execution-contract.ts`、四家 provider adapter、`src/claude-tui-transport.ts`、`src/claude-tui-lifecycle.ts`、`src/pi-host.ts`、`src/pi-host-protocol.ts`、`src/local-console/execution-driver.ts`。
...
![claude-tui-runtime](claude-tui-runtime.svg)
