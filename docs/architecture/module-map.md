...
- 职责边界：`src/**`、`desktop/src/**` 与 `packages/console-ui/src/**` 的生产 TS/TSX 文件必须唯一归入 view / application / domain / adapter。domain 是可直接单测的纯闭包；application 编排端口与时序；adapter 承接 fs、SQLite、provider CLI、HTTP/IPC；view 只映射状态到显示。composition root 由 exact allowlist 管理，存量债务必须绑定移除 change，新增文件、stale debt、stale permit 与未登记 root 均 fail closed。
