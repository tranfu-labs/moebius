...
- 职责边界：`src/**`、`desktop/src/**` 与 `packages/console-ui/src/**` 的生产 TS/TSX 文件必须唯一归入 view / application / domain / adapter。domain 是可直接单测的纯闭包；application 编排端口与时序；adapter 承接 fs、SQLite、provider CLI/API、HTTP/IPC；view 只映射状态到显示。最终机械基线为 600 个生产文件（view 91 / application 186 / domain 199 / adapter 125）、file/dependency debt 0、composition root allowlist 12、exact permit 281；新增文件、stale debt、stale permit 与未登记 root 均 fail closed。
