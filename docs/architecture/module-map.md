...
- 入口：`src/local-console/server.ts`、`src/local-console/runtime.ts`、`src/local-console/execution-context.ts`、`src/local-console/execution-driver.ts`、`src/local-console/codex-resume.ts`、`src/local-console/codex-thread-link.ts`、`src/local-console/prompt.ts`、`src/local-console/store.ts`、`src/local-console/attachments.ts`、`src/local-console/output-tail.ts`；Codex 适配在 `src/codex.ts`，Kimi ACP 适配在 `src/kimi.ts`。
