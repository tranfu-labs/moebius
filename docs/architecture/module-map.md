...
- 入口：`src/local-console/server.ts`、`src/local-console/runtime.ts`、`src/local-console/run-activity.ts`、`src/local-console/execution-context.ts`、`src/local-console/execution-driver.ts`、`src/local-console/codex-resume.ts`、`src/local-console/codex-thread-link.ts`、`src/local-console/codex-rollout.ts`、`src/local-console/process-history.ts`、`src/local-console/process-event-projector.ts`、`src/local-console/prompt.ts`、`src/local-console/store.ts`、`src/local-console/attachments.ts`、`src/local-console/output-tail.ts`；Codex 适配与共享 rollout identity resolver 在 `src/codex.ts`、`src/codex-rollout.ts`（local 路径仅兼容 re-export），Kimi ACP 适配及其受管运行目录隔离在 `src/kimi.ts`、`src/kimi-runtime-home.ts`。
