...
说明：这是 `spec-delta/local-console/spec.md` 的 OpenSpec CLI 可验证镜像，两者语义保持一致。

## MODIFIED Requirements

### Requirement: 来源胶囊序列化为用户消息来源块

The local console MUST treat each visible text-fragment string as its only Agent input payload.
...
The local console reference-text API MUST return safe visible navigation-link text without granting runtime source access.
...
The local console MUST use persisted visible message text as the only source text for retries and recovery.
