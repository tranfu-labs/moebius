...
源码文本/文件路径快照测试不进入上述“集成测试降级为纯测试”账。20/30/40 批若因结构迁移暴露此类
镜像断言，先判定其架构约束是否已由具体 `[IB:*]` 接管：已接管则即时删除，并记录删除 test name、
镜像判据和接管门禁；未接管则保留，且不得靠抄写新路径或新实现文本修绿。50 批汇总时为这类删除
单列“镜像测试 → 机制化门禁”账，不与集成降级账混算。
