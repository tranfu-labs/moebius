...
选择题继续使用既有 `options[]` 数据形状、原生 radio / checkbox 与产品追加的「自己写」项，只把纯模型允许的预设选项上限从 2 调整到 4。协议解析继续复用 `isRenderableAgentForm`，Prompt 中的数量声明与模型常量同步；不新增第二套校验。四个预设选项加「自己写」最多五项，纵向空间沿用既有题目区内部滚动，不改变卡片宽度、宿主高度或状态模型。
...
- **不做 `role="radiogroup"` 手写 roving tabindex**：`agent-portrait-picker` 那种写法是为了把 36 个候选收成一个 tab stop；这里一题最多五项，原生 radio 分组已经承载分组与方向键行为，自己实现只会多一处能写错的地方。
