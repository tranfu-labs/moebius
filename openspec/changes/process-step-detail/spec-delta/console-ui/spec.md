...
### Requirement: 过程步骤位于实时输出之前并提供克制的展开反馈

Source: docs/product/pages/agent-conversation.md#过程步骤

系统 MUST 在运行记录的角色信息之后、流式正文或最新活动之前显示过程步骤。过程区与单步详情的展开／收起 MUST 使用可打断的令牌化容器过渡，且 MUST 在减弱动效模式下直接切换到终态。系统 MUST NOT 缩放文字、使用弹跳动效，或让隐藏内容产生可聚焦控件。

#### Scenario: 工具轨迹与正在输出的正文同时存在

- **GIVEN** 一条运行记录已有过程步骤且 Agent 正在输出正文
- **WHEN** 用户查看该记录
- **THEN** 角色信息之后先显示过程步骤，再显示正在输出的正文

#### Scenario: 快速反向展开过程区

- **GIVEN** 用户正在展开或收起过程区
- **WHEN** 用户在动画结束前反向操作
- **THEN** 过渡从当前视觉进度继续到新状态，不从首帧跳变
- **AND** 减弱动效模式下直接呈现新状态
