...
failure code 在异步结果提交为可见错误时使用最新 translator 解析，因此新产生的失败会采用提交时的当前语言；
已经提交并显示的错误仍是渲染后的字符串，不会在后续 locale 切换时自动重译。若要让存量错误实时重译，必须
把 failure code 保存在状态中并将翻译职责继续推到 view，这会穿透现有 console-ui prop 契约并突破本批
500 行硬停点。50 批明确接受该限制；未来若产品要求错误文案随语言实时切换，另建 change 处理。
