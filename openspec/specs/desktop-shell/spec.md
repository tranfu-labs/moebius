...
四步 MUST 共享相同的顶部标题区与底部操作条。普通步骤主体内容宽度 MUST 受 `max-w-lg` 约束；第 2 步 AI 团队设计器打开时 MUST 作为唯一例外随窗口响应式放宽到最大约 `780px`、增高到最大约 `720px`，关闭后恢复普通窄栏。步骤切换不得改变顶底栏的结构位置。
...
- **AND** 普通步骤主体不超过 `max-w-lg`
- **AND** AI 团队设计器打开时不超过约 `780px × 720px`。
