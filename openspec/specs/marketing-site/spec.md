...
被引用的原始材料是例外：对照段的题面原文与运行记录里的 Agent 原话 MUST 保持来源语言，MUST NOT 翻译或改写后仍以原话呈现。这类区块 MUST 标记 `data-verbatim`，并 MUST 自报 `lang`（标在区块本身，或在语言混杂时标在每一条上）。页面 MAY 在原话旁给出本页语言的概述，但概述 MUST 与原话在视觉上可区分，且 MUST NOT 冒充原话。
...
- GIVEN 对照段的某一步运行是用中文跑的
- WHEN 访客在英文页选中这一步
- THEN 这一步的原话按中文原样呈现，不被翻译或改写
- AND 该区块自报 `lang`，并与旁边英文的概述在视觉上可区分
...
### Scenario: 访客核对两份产物各自的代价

- GIVEN 访客读完对照段的两份产物
- WHEN 继续往下看同一条时间轴上的两次运行
- THEN 能一眼看出两次运行的总长与各步的切分
- AND 选中任一步能看到它的角色、用时、往返与工具调用次数、花费和当时的原话
- AND 选中复核那一步能看到它把题面拆成了哪些条目、用什么手段核的
- AND 能看到这些数字来自哪两次运行、花费按什么价目折算
...
- 对照段 MUST 在两份产物之后给出两次运行的记录，并 MUST 把两次运行画在同一条时间轴上：每一步是轴上的一段，段长 MUST 与该步的实际耗时成比例；两条轴 MUST 各给出墙钟时间、花费与生成 token 的合计。
- 运行记录的每一步 MUST 可以选中，选中后 MUST 展开这一步的角色、用时、模型往返与工具调用次数、花费，以及这个 Agent 在这一步写下的**全部消息原话**——逐条、带时间、未经编辑。原话 MAY 折叠掉 Agent 贴进对话的产物源码，折叠处 MUST 标明折叠了什么。复核那一步 MUST 另给出它对着题面核对的条目与核对手段。步骤切换 MUST 在没有 JavaScript 时可用，并 MUST 可用键盘操作。
- 运行记录里的每个数字 MUST 取自这两次运行的实际会话记录，花费 MUST 是这些 token 按所用模型的公开 API 价目折算；页面 MUST 说明数字的来源、折算口径，并 MUST 说明这是一道题的两次运行而不是基准测试。
