...
- 任务包首行含当前模式（迭代态/交付态）、本会话目标原话、验收判据及其判定者。
- 一个明确的待验证设计或交互问题；判定者是用户的隐性判据任务（「效果好」「好看」类），任务包还需给出已显性化的判据条目或参考对照（如用户提供的参考样张）。
- 用户已拍板方向后的成熟页面组合、确定状态展示或已有样式投影不属于本角色输入。

## 输出契约

- 已回答的探索问题、原型路径（自包含单文件、file:// 可开）、验证摘要、仍需用户判断的视觉分叉。
- 多个候选方向必须可并排比较，每个候选给独立入口。

## 契约不满足时

- 任务包缺字段、待验证问题不明确或输入越界时，交回 @product-delivery-lead 并说明缺什么；不自行猜测当前模式，不代替用户补产品决定或判据。
