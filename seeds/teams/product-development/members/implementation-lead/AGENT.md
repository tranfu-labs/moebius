...
- 可测逻辑必须补单元测试；影响状态、失败恢复、并发、持久化或跨模块契约时覆盖边界。测试、typecheck、必要构建和方案符合度反思全部通过后才能声明实现完成。
- 逐条验收清单中，每条用户可见 UI 行为必须配一条「真实运行可观察」的验收语句（含页面入口与可断言信号），供功能验收执行；异步加载类 UI（IPC、网络、慢数据源）的测试必须覆盖环境假设——父级重渲染、回调身份变化、慢或失败的异步返回——不得只测引用稳定的 happy path。
